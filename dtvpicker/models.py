...
class SubEvent(models.Model):
    ...
    start_date_and_time = models.DateTimeField(blank=True, null=True,
            help_text='When will this sub-event start? (yyyy-mm-dd hh:mm:ss)'
            )
    end_date_and_time = models.DateTimeField(blank=True, null=True,
            help_text='When will this sub-event end? (yyyy-mm-dd hh:mm:ss)'
            )
    venue = models.CharField(max_length=64, blank=True, null=True,
                             help_text='Where will this sub-event be held?'
    ...
    def checkSubEventClash(
        ...
        for subEvent in subEventList:
            ...
            subEventClashMsgs.append(u'%s is unavailable from %s to %s.'
                     % (subEvent.venue, subEvent.start_date_and_time,
    ...
    def clean(self):
        ...
        if self.start_date_and_time and self.end_date_and_time:
            ...
            if self.venue:
                ...
                subEventClashErrorMsg = \
                    self.checkSubEventClash(self.venue,
                        self.start_date_and_time,
                        self.end_date_and_time)
        ...
        if self.title:
            ...
            for subEvent in subEventList:
                if subEvent.title == self.title:
                    errors.append(u'You already have a sub-event with the name "%s".'
                                   % subEvent.title)
                    break
