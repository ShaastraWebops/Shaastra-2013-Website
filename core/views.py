...
def home(request):
    ...
    return render_to_response('core/home.html', locals(),
...
def addevent(request):
    ...
    if request.user.get_profile().is_core is False \
        and not request.user.get_profile().is_coord_of:
        return HttpResponseRedirect(settings.SITE_URL)
    if request.method == 'POST':
        filename = request.META['HTTP_X_FILE_NAME']
        event_id = request.META['HTTP_X_EVENT_ID']
        direc = \
            os.path.join('/home/shaastra/public_html/2013/media/events'
                         , event_id)
    ...
    return render_to_response('ajax/core/addevent.html', locals(),
                              context_instance=RequestContext(request))
