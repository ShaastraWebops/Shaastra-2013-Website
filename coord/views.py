...
class BaseView(object):
    ...
    def __call__(self, request, **kwargs):
        ...
        if not self.authenticate_req(request):
...
class CoordDashboard(BaseView):
    ...
    def handle_GET(self, request, **kwargs):
        ...
        return render_to_response('coord/dashboard.html', locals(),
...
class Registrations(BaseView):
    ...
    def handle_GET(self, request, **kwargs):
        if request.user.get_profile().is_coord_of.has_questionnaire:
            return HttpReponse("Click on the link 'Submissions' to view the registrations"
                               )
        registrations = \
            request.user.get_profile().is_coord_of.participants.all()
        return render_to_response('ajax/coord/registrations.html',
                                  locals(),
                                  context_instance=RequestContext(request))
...
class Questions(BaseView):
    ...
    def handle_GET(self, request, **kwargs):
        if not request.user.get_profile().is_coord_of.has_questionnaire:
            return HttpResponse("This event doesn't have a questionnaire"
                                )
...
class MobApp(BaseView):
    ...
    def handle_GET(self, request, **kwargs):
        ...
        return render_to_response('ajax/coord/add_edit_mobapptab.html',
                                  locals(),
                                  context_instance=RequestContext(request))
...
def EditUpdate(request, id=0):
    ...
    return render_to_response('ajax/coord/editupdate.html', locals(),
                              context_instance=RequestContext(request))
...
#        return HttpResponseRedirect(settings.SITE_URL)
    ...
    return render_to_response('ajax/core/editevent.html', locals(),
                              context_instance=RequestContext(request))
